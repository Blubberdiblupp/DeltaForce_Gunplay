/**
 * DeltaForceNetworkOptimizer.c - hardcoded gunplay recalculation filter.
 * 
 * Only truly cosmetic items are filtered (wraps, camo, slings).
 * All gameplay-relevant attachments trigger recalculation.
 */

class DeltaForceNetworkOptimizer
{
	protected static ref DeltaForceNetworkOptimizer m_Instance;
	...
	static DeltaForceNetworkOptimizer GetInstance()
	{
		if (!m_Instance)
		{
			m_Instance = new DeltaForceNetworkOptimizer();
			m_Instance.Init();
		}
		return m_Instance;
	}
	
	void DeltaForceNetworkOptimizer()
	{
	...
	void Init()
	{
		...
		Print("[DeltaForce Gunplay] Network optimizer initialized");
		Print("[DeltaForce Gunplay] Stat-affecting attachments: " + m_StatAffectingAttachments.Count());
		Print("[DeltaForce Gunplay] Cosmetic attachments: " + m_CosmeticAttachments.Count());
	}
	...
	protected void InitStatAffectingAttachments()
	{
		...
		m_StatAffectingAttachments.Insert("My_DF_Weapons_");
		m_StatAffectingAttachments.Insert("My_DF_Weapons");
