class DeltaForceFirearmsMechanics
{
	...
	static vector CalculateHipFireSpread(Weapon_Base weapon, PlayerBase player, vector aimDirection)
	{
		...
		float spreadAngle = config.HipFireAccuracyDegradation * weapon.GetDeltaForceHipFireSpreadModifier();
		...
		if (config.DebugMode)
		{
			Print("[DeltaForce Gunplay] Hip Fire Spread: " + spreadVector + " (Original: " + aimDirection + ")");
		}
	...
	static vector ApplyHipFireCorrection(Weapon_Base weapon, PlayerBase player, vector originalDirection)
	{
		...
		if (DeltaForceVehicleCombatHandler.IsPlayerInVehicle(player))
		{
			if (config.DebugMode)
			{
				string vehicleType = DeltaForceVehicleCombatHandler.GetVehicleType(player);
				Print("[DeltaForce Gunplay] Player in " + vehicleType + " - Hip-Fire Nerf SKIPPED");
			}
			return originalDirection;
		}
		...
		if (config.DebugMode)
		{
			Print("[DeltaForce Gunplay] Hip Fire Correction Applied");
