...
class DeltaForceHipFireCrosshair: Managed
{
	protected const float MORTYS_CROSSHAIR_BASE_SIZE = 64.0;
	protected const float MORTYS_CROSSHAIR_MIN_SIZE = 40.0;
	protected const float MORTYS_CROSSHAIR_MAX_SIZE = 88.0;
	protected const float MORTYS_PROJECT_DISTANCE = 100.0;
	protected const float MORTYS_SMOOTHNESS = 0.035;
	...
	void DeltaForceHipFireCrosshair()
	{
		m_Root = GetGame().GetWorkspace().CreateWidgets("DeltaForce_Gunplay/GUI/layouts/DeltaForceHipFireCrosshair.layout");
		if (!m_Root)
		{
			return;
		}
		
		m_DynamicRoot = m_Root.FindAnyWidget("DeltaForceHipFireCrosshairRoot");
		m_DynamicImage = ImageWidget.Cast(m_Root.FindAnyWidget("DeltaForceHipFireCrosshairImage"));
	...
	void OnUpdate(float timeslice)
	{
		...
		m_ScreenPosition[0] = Math.SmoothCD(m_ScreenPosition[0], targetPosition[0], m_VelocityX, MORTYS_SMOOTHNESS, 1000.0, timeslice);
		m_ScreenPosition[1] = Math.SmoothCD(m_ScreenPosition[1], targetPosition[1], m_VelocityY, MORTYS_SMOOTHNESS, 1000.0, timeslice);
	...
	protected bool GetWeaponProjectedPosition(Weapon_Base weapon, out vector worldPosition)
	{
		...
		direction.Normalize();
		vector traceEnd = endPoint + (direction * MORTYS_PROJECT_DISTANCE);
		vector contactDir;
	...
	{
		float hipFireModifier = Math.Clamp(weapon.GetDeltaForceHipFireSpreadModifier(), 0.45, 1.45);
		return Math.Clamp(MORTYS_CROSSHAIR_BASE_SIZE * hipFireModifier, MORTYS_CROSSHAIR_MIN_SIZE, MORTYS_CROSSHAIR_MAX_SIZE);
	}
...
{
	protected ref DeltaForceHipFireCrosshair m_DeltaForceHipFireCrosshair;
	
	void IngameHud()
	{
		m_DeltaForceHipFireCrosshair = new DeltaForceHipFireCrosshair();
	}
	...
	override void Update(float timeslice)
	{
		...
		if (m_DeltaForceHipFireCrosshair)
		{
			m_DeltaForceHipFireCrosshair.OnUpdate(timeslice);
		}
	}
}
